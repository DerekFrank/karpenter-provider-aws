...
* **Breaking:** [Node Auto Repair]({{<ref "../concepts/disruption#node-auto-repair" >}}) (alpha, `NodeRepair` feature gate) is now a graceful disruption method. Instead of forcefully terminating an unhealthy node, Karpenter pre-spins a replacement, respects [NodePool Disruption Budgets]({{<ref "../concepts/disruption#nodepool-disruption-budgets" >}}) under the new `Unhealthy` reason, and drains the node while respecting PDBs, bounded by the NodeClaim's `terminationGracePeriod`. If you enable `NodeRepair`, set `terminationGracePeriod` on your NodePools so that blocking PDBs can't stall repairs indefinitely, and use the new `karpenter.sh/do-not-repair: "true"` node annotation to opt individual nodes out of repair. With the default budget (`nodes: 10%`), Karpenter repairs at most 10% of a NodePool's nodes at a time. The CRD adds `Unhealthy` to the budget `reasons` enum, so upgrade your CRDs alongside the controller. See [CRD Upgrades](#crd-upgrades) above.
